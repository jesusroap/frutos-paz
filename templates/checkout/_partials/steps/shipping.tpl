      ...
      >
        <div class="form-fields">
          {block name='delivery_options'}
            <span>{l s='Select an option' d='Shop.Theme.Checkout'}:</span>
            <div id="delivery-option-list-custom" class="delivery-options">
              {foreach from=$delivery_options item=carrier key=carrier_id}
                  <div class="row delivery-option" {if $carrier.name == 'Por calcular'} style="display: none"{/if}>
                    <div class="col-sm-1">
                      <span class="custom-radio float-xs-left">
                        <input type="radio" name="delivery_option[{$id_address}]" id="delivery_option_{$carrier.id}" value="{$carrier_id}"{if $delivery_option == $carrier_id && $carrier.name != 'Por calcular'} checked{/if} {if $carrier.name == 'Por calcular'} disabled{/if} required>
                        <span></span>
                      </span>
                    </div>
